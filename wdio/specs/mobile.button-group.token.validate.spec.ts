...
const widgetKey: Widget = 'button-group';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Button Group Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-button-group.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/button-group-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `button-group-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload['button-group']?.mapping) {
    console.log(`📂 [button-group] Extracting tokens from mapping structure`);

    // For button-group, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const buttonGroupMapping = appliedPayload['button-group'].mapping;

    // Use the first appearance and variant as defaults for button-group structure
    ...
    extractTokens(buttonGroupMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in button-group payload`);
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Button Group page)', async function () {
    
    

    const screenshotName = 'button-group-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (Button Group page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'button-group-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Button Group Token Validation Tests Complete');
