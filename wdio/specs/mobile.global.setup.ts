...
export default async function mobileGlobalSetup() {
  ...
  try {
    ...
    // ========== PHASE 4: Pre-fetch all widget styles to disk cache ==========
    console.log('\n📦 PHASE 4: Pre-fetching widget styles to disk cache');
    console.log('='.repeat(80));
    console.log('   This avoids per-widget BrowserStack sessions during test execution.\n');

    const { createAndroidSession, createIOSSession } = await import('../utils/sessionFactory');
    const { MobileWidgetPage } = await import('../pages/MobileWidget.page');
    const { getStudioWidgetNameForVariant } = await import('../utils/mobileWidgetVariantCsv');

    // Collect unique (widget, studioWidgetName) pairs from the token-variant pairs
    const uniqueWidgetStyles = new Map<string, { widget: string; studioWidgetName: string; variantName: string }>();
    for (const pair of tokenVariantPairs) {
      const w = pair.item.widget;
      const variantName = `${w}-${pair.item.appearance}-${pair.item.variant}-${pair.item.state}`;
      const studioWidgetName = getStudioWidgetNameForVariant(w as any, variantName);
      if (!studioWidgetName) continue;
      const key = `${w}::${studioWidgetName}`;
      if (!uniqueWidgetStyles.has(key)) {
        uniqueWidgetStyles.set(key, { widget: w, studioWidgetName, variantName });
      }
    }

    console.log(`   Found ${uniqueWidgetStyles.size} unique widget-variant styles to pre-fetch\n`);

    const stylesCacheDir = path.join(process.cwd(), 'artifacts', 'mobile-styles');
    const dummyBaselineApps = { android: '', ios: '' };

    const prefetchForPlatform = async (platform: 'android' | 'ios') => {
      const appUrl = platform === 'android' ? actualBrowserStack.android : actualBrowserStack.ios;
      if (!appUrl) {
        console.log(`   ⏭  Skipping ${platform} styles pre-fetch (no app URL)`);
        return;
      }

      let browser: any;
      try {
        const createSession = platform === 'android' ? createAndroidSession : createIOSSession;
        browser = await createSession({
          sessionName: `${platform} Styles Pre-fetch`,
          type: 'actual',
          runLocal: false,
          widgetKey: 'accordion' as any,
          baselineApps: dummyBaselineApps,
          actualApps: { android: actualBrowserStack.android || '', ios: actualBrowserStack.ios || '' },
        });
        ...
        for (const [, entry] of uniqueWidgetStyles) {
          const diskPath = path.join(stylesCacheDir, entry.widget, `${entry.studioWidgetName}.styles.json`);
          ...
          try {
            ...
            fetched++;
            if (fetched % 10 === 0) {
              console.log(`   📦 ${platform}: ${fetched} styles fetched...`);
            }
          } catch (err: any) {
            console.warn(`   ⚠️  ${platform} failed to fetch styles for ${entry.widget}/${entry.studioWidgetName}: ${err.message}`);
          }
        }

        console.log(`   ✅ ${platform}: ${fetched} styles fetched, ${skipped} already cached`);
      } catch (err: any) {
        console.warn(`   ⚠️  ${platform} styles pre-fetch failed: ${err.message}`);
      } finally {
        if (browser) {
    ...
    await Promise.all([
      runAndroid ? prefetchForPlatform('android') : Promise.resolve(),
      runIOS ? prefetchForPlatform('ios') : Promise.resolve(),
    ]);

    console.log('✅ Styles pre-fetch complete — specs will load from disk cache\n');
