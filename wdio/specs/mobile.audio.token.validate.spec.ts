...
const widgetKey: Widget = 'audio';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Audio Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-audio.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/audio-widget-variants.csv');
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (audio page)', async function () {
    
    

    const screenshotName = 'audio-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (audio page)', async function () {
