...
const widgetKey: Widget = 'spinner';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Spinner Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-spinner.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/spinner-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `spinner-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.spinner?.mapping) {
    console.log(`📂 [spinner] Extracting tokens from mapping structure`);

    // For spinner, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const spinnerMapping = appliedPayload.spinner.mapping;

    // Use the first appearance and variant as defaults for spinner structure
    ...
    extractTokens(spinnerMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in spinner payload`);
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Spinner page)', async function () {
    
    

    const screenshotName = 'spinner-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (spinner page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'spinner-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Spinner Token Validation Tests Complete');
