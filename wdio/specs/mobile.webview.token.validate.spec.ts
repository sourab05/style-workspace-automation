...
const widgetKey: Widget = 'webview';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Webview Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-webview.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/webview-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `webview-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.webview?.mapping) {
    console.log(`📂 [webview] Extracting tokens from mapping structure`);

    // For webview, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const webviewMapping = appliedPayload.webview.mapping;

    // Use the first appearance and variant as defaults for webview structure
    ...
    extractTokens(webviewMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in webview payload`);
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Webview page)', async function () {
    
    

    const screenshotName = 'webview-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (webview page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'webview-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Webview Token Validation Tests Complete');
