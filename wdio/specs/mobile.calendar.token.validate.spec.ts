...
const widgetKey: Widget = 'calendar';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Calendar Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-calendar.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/calendar-widget-variants.csv');
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (calendar page)', async function () {
    
    

    const screenshotName = 'calendar-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (calendar page)', async function () {
