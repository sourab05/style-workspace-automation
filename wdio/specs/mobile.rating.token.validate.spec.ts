...
const widgetKey: Widget = 'rating';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Rating Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-rating.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/rating-widget-variants.csv');
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (rating page)', async function () {
    
    

    const screenshotName = 'rating-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (rating page)', async function () {
