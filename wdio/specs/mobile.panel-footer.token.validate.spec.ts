...
const widgetKey: Widget = 'panel-footer';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Panel Footer Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-panel-footer.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/panel-footer-widget-variants.csv');
  ...
  if (appliedPayload['panel-footer']?.mapping) {
    const appearance = widgetConfig.appearances[0];
    const variant = widgetConfig.variants[appearance]?.[0] || 'standard';
    extractTokens(appliedPayload['panel-footer'].mapping, appearance, variant, []);
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (panel-footer page)', async function () {
    
    

    const screenshotName = 'panel-footer-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (panel-footer page)', async function () {
