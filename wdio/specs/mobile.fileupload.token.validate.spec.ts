...
const widgetKey: Widget = 'fileupload';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - FileUpload Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-fileupload.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/fileupload-widget-variants.csv');
  ...
  if (appliedPayload.fileupload?.mapping) {
    ...
    extractTokens(appliedPayload.fileupload.mapping, appearance, variant, []);
  }
