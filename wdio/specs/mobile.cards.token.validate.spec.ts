...
const widgetKey: Widget = 'cards';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Cards Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-cards.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/cards-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `cards-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.card?.appearances) {
    // Iterate through configured appearances
    for (const appearance of widgetConfig.appearances) {
      const appearanceData = appliedPayload.card.appearances[appearance];
      ...
      // For cards, extract tokens from the mapping object within each appearance
      if (appearanceData.mapping) {
        ...
        console.log(`   ⚠️  No mapping found for appearance '${appearance}'`);
      }
    }
  }
      ...
      : {
        ...
        capabilities: {
          ...
          'bstack:options': {
            ...
            buildName: `Mobile Token ${type === 'baseline' ? 'Baseline' : 'Validation'} - Android (cards)`,
            sessionName,
            debug: true,
            networkLogs: true,
            video: true,
  ...
  appliedPairs.forEach((pair) => {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      it(`Android: validate ${tokenRef} @ ${variantName} [${propertyPath.join('.')}]`, async function () {
        ...
        await verifier.verifyTokenApplication(
          ...
          'cards-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
    ...
    after(async function () {
      ...
      console.log('\n' + '='.repeat(80));
      console.log('📱 Mobile Cards Token Validation Tests Complete');
