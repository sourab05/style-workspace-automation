...
const widgetKey: Widget = 'label';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Label Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-label.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/label-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `label-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.label?.appearances) {
    // Iterate through configured appearances
    for (const appearance of widgetConfig.appearances) {
      const appearanceData = appliedPayload.label.appearances[appearance];
      ...
      // For 'default' appearance, use 'size' variantGroup
      // For 'text' and 'label' appearances, use 'status' variantGroup
      const variantGroupKey = appearance === 'default' ? 'size' : 'status';

      if (appearanceData.variantGroups?.[variantGroupKey]) {
        for (const variant of variantsForAppearance) {
          const variantData = appearanceData.variantGroups[variantGroupKey][variant];
        ...
        console.log(`   ⚠️  No ${variantGroupKey} variantGroup found for appearance '${appearance}'`);
      }
    }
  }
  ...
  appliedPairs.forEach((pair) => {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      it(`Android: validate ${tokenRef} @ ${variantName} [${propertyPath.join('.')}]`, async function () {
        ...
        await verifier.verifyTokenApplication(
          ...
          'label-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
    ...
    after(async function () {
      ...
      console.log('\n' + '='.repeat(80));
      console.log('📱 Mobile Label Token Validation Tests Complete');
