...
const widgetKey: Widget = 'video';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Video Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-video.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/video-widget-variants.csv');
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (video page)', async function () {
    
    

    const screenshotName = 'video-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (video page)', async function () {
  ...
  appliedPairs.forEach((pair) => {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(browser, 'android', widgetKey, variantName, tokenRef, {}, 'video-page', propertyPath);
      });
      }
      afterEach(function () {
        try {
