...
const widgetKey: Widget = 'icon';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Icon Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-icon.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/icon-widget-variants.csv');
  ...
  if (appliedPayload.icon?.appearances) {
    // Iterate through configured appearances
    for (const appearance of widgetConfig.appearances) {
      const appearanceData = appliedPayload.icon.appearances[appearance];
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (icon page)', async function () {
    
    

    const screenshotName = 'icon-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (icon page)', async function () {
