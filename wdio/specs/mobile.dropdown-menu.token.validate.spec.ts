...
const widgetKey: Widget = 'dropdown-menu';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Dropdown Menu Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-dropdown-menu.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/dropdown-menu-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `dropdown-menu-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  // Note: payload key is 'dropdown' not 'dropdown-menu' (see getWidgetKey in generator.ts)
  if (appliedPayload.dropdown?.mapping) {
    console.log(`📂 [dropdown-menu] Extracting tokens from mapping structure`);

    // For dropdown-menu, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const dropdownMenuMapping = appliedPayload.dropdown.mapping;

    // Use the first appearance and variant as defaults for dropdown-menu structure
    ...
    extractTokens(dropdownMenuMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in dropdown-menu payload`);
  }
  ...
  appliedPairs.forEach((pair) => {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      it(`Android: validate ${tokenRef} @ ${variantName} [${propertyPath.join('.')}]`, async function () {
        ...
        await verifier.verifyTokenApplication(
          ...
          'dropdown-menu-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
    ...
    after(async function () {
      ...
      console.log('\n' + '='.repeat(80));
      console.log('📱 Mobile Dropdown Menu Token Validation Tests Complete');
