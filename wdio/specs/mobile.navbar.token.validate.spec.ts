...
const widgetKey: Widget = 'navbar';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Navbar Widget', function () {
    ...
    const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-navbar.json');
    ...
    const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/navbar-widget-variants.csv');
    ...
    function extractTokens(
        ...
        if ('value' in obj && typeof obj.value === 'string') {
            ...
            const variantName = `navbar-${appearance}-${variant}-${stateSuffix}`;
            appliedPairs.push({
                tokenRef: obj.value,
                variantName,
    ...
    if (appliedPayload.navbar?.mapping) {
        console.log(`📂 [navbar] Extracting tokens from mapping structure`);

        const navbarMapping = appliedPayload.navbar.mapping;
        ...
        extractTokens(navbarMapping, defaultAppearance, defaultVariant, []);
    } else {
        console.log(`⚠️  No mapping found in navbar payload`);
    }
    ...
    baselineScreenshotIt('android')('Android baseline vs actual screenshot (Navbar page)', async function () {
        
        

        const screenshotName = 'navbar-page';
    ...
    baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (Navbar page)', async function () {
    ...
    after(async function () {
        ...
        console.log('\n' + '='.repeat(80));
        console.log('📱 Mobile Navbar Token Validation Tests Complete');
