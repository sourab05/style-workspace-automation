...
const widgetKey: Widget = 'form-wrapper';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Form-Wrapper Widget', function () {
    ...
    const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-form-wrapper.json');
    ...
    const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/form-wrapper-widget-variants.csv');
    ...
    function extractTokens(
        ...
        if ('value' in obj && typeof obj.value === 'string') {
            ...
            const variantName = `form-wrapper-${appearance}-${variant}-${stateSuffix}`;
            appliedPairs.push({
                tokenRef: obj.value,
                variantName,
    ...
    if (appliedPayload.form?.mapping) {
        console.log(`📂 [form-wrapper] Extracting tokens from mapping structure`);

        const formMapping = appliedPayload.form.mapping;

        // Use the first appearance and variant as defaults
        const defaultAppearance = widgetConfig.appearances[0] || 'standard';
        const defaultVariant = widgetConfig.variants[defaultAppearance]?.[0] || 'default';

        console.log(`   ✓ Extracting tokens for ${defaultAppearance}.${defaultVariant}`);
        extractTokens(formMapping, defaultAppearance, defaultVariant, []);
    } else {
        console.log(`⚠️  No mapping found in form-wrapper payload`);
    }
    ...
    baselineScreenshotIt('android')('Android baseline vs actual screenshot (Form-Wrapper page)', async function () {
        
        

        const screenshotName = 'form-wrapper-page';
    ...
    baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (Form-Wrapper page)', async function () {
    ...
    appliedPairs.forEach((pair) => {
        ...
        describe(`Token Check: ${tokenRef}`, function () {
            if (shouldRunAndroid) {
    ...
    after(async function () {
        ...
        console.log('\\n' + '='.repeat(80));
        console.log('📱 Mobile Form-Wrapper Token Validation Tests Complete');
