...
const widgetKey: Widget = 'login';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Login Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-login.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/login-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `login-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.login?.mapping) {
    console.log(`📂 [login] Extracting tokens from mapping structure`);

    // For login, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const loginMapping = appliedPayload.login.mapping;

    // Use the first appearance and variant as defaults for login structure
    ...
    extractTokens(loginMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in login payload`);
  }
      ...
      : {
        ...
        capabilities: {
          ...
          'bstack:options': {
            ...
            buildName: `Mobile Token ${type === 'baseline' ? 'Baseline' : 'Validation'} - Android (Login)`,
            sessionName,
            debug: true,
            networkLogs: true,
            video: true,
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Login page)', async function () {
    
    

    const screenshotName = 'login-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (Login page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'login-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Login Token Validation Tests Complete');
