...
const widgetKey: Widget = 'slider';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Slider Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-slider.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/slider-widget-variants.csv');
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (slider page)', async function () {
    
    

    const screenshotName = 'slider-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (slider page)', async function () {
