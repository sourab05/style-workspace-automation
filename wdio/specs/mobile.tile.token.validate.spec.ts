...
const widgetKey: Widget = 'tile';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Tile Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-tile.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/tile-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `tile-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Tile page)', async function () {
    
    

    const screenshotName = 'tile-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (tile page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'tile-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Tile Token Validation Tests Complete');
