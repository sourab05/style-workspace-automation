...
const widgetKey: Widget = 'radioset';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Radioset Widget', function () {
  ...
  const payloadWidgetKey = getWidgetKey(widgetKey);
  ...
  if (appliedPayload[payloadWidgetKey]?.mapping) {
    console.log(`📂 [${widgetKey}] Extracting tokens from direct mapping`);
    extractTokens(appliedPayload[payloadWidgetKey].mapping, 'standard', 'standard', []);
  } else if (appliedPayload[payloadWidgetKey]?.appearances) {
    const apps = appliedPayload[payloadWidgetKey].appearances;
    for (const [appName, appData] of Object.entries(apps)) {
      if ((appData as any).mapping) {
        extractTokens((appData as any).mapping, appName, 'standard', []);
      }
    }
  }
