...
const widgetKey: Widget = 'progress-circle';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Progress Circle Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-progress-circle.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/progress-circle-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `progress-circle-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  const widgetPayload = appliedPayload[widgetKey];
  if (widgetPayload?.appearances) {
    // Iterate through configured appearances
    for (const appearance of widgetConfig.appearances) {
      const appearanceData = widgetPayload.appearances[appearance];
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Progress Circle page)', async function () {
    
    

    const screenshotName = 'progress-circle-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (progress-circle page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'progress-circle-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Progress Circle Token Validation Tests Complete');
