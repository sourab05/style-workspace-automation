...
const widgetKey: Widget = 'panel';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Panel Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-panel.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/panel-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `panel-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.panel?.appearances) {
    // Iterate through configured appearances
    for (const appearance of widgetConfig.appearances) {
      const appearanceData = appliedPayload.panel.appearances[appearance];
  ...
  // Count token validation results once per Mocha test. Screenshot/baseline tests are excluded.
  afterEach(function () {
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (panel page)', async function () {
    
    

    const screenshotName = 'panel-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (panel page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'panel-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Panel Token Validation Tests Complete');
