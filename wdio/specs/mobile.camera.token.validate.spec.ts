...
const widgetKey: Widget = 'camera';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Camera Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-camera.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/camera-widget-variants.csv');
