...
const widgetKey: Widget = 'button';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Button Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-button.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/button-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `button-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.btn?.appearances) {
    // Iterate through configured appearances
    for (const appearance of widgetConfig.appearances) {
      const appearanceData = appliedPayload.btn.appearances[appearance];

      if (!appearanceData) {
        console.log(`⚠️  Appearance '${appearance}' not found in payload`);
        continue;
      }

      // Get variants for this appearance from config
      const variantsForAppearance = widgetConfig.variants[appearance] || [];

      if (variantsForAppearance.length === 0) {
        console.log(`⚠️  No variants configured for appearance '${appearance}'`);
        continue;
      }

      console.log(`📂 [${appearance}] Processing variants: ${variantsForAppearance.join(', ')}`);

      // Navigate through variantGroups.status.{variantName}
      if (appearanceData.variantGroups?.status) {
        for (const variant of variantsForAppearance) {
          const variantData = appearanceData.variantGroups.status[variant];

          if (!variantData) {
            console.log(`   ⚠️  Variant '${variant}' not found in payload for appearance '${appearance}'`);
            continue;
          }

          console.log(`   ✓ Extracting tokens for ${appearance}.${variant}`);
          extractTokens(variantData, appearance, variant, []);
        }
      } else if (appearanceData.mapping) {
        const variant = variantsForAppearance[0] || 'standard';
        console.log(`   ✓ Extracting tokens from mapping for ${appearance}.${variant}`);
        extractTokens(appearanceData.mapping, appearance, variant, []);
      }
    }
  }
  ...
  async function createAndroidOLD_UNUSED(sessionName: string, type: 'baseline' | 'actual'): Promise<Browser> {
    const appPath = type === 'baseline'
      ? (process.env.ANDROID_BASELINE_APK_PATH || baselineApps.android)
      : (process.env.ANDROID_ACTUAL_APK_PATH || actualApps.android);

    const isLocal = runLocal;
      ...
      : {
        ...
        capabilities: {
          ...
          'bstack:options': {
            ...
            buildName: `Mobile Token ${type === 'baseline' ? 'Baseline' : 'Validation'} - Android (Button)`,
            sessionName,
            debug: true,
            networkLogs: true,
            video: true,
  ...
  async function createIOSOLD_UNUSED(sessionName: string, type: 'baseline' | 'actual'): Promise<Browser> {
    const appPath = type === 'baseline'
  ...
  appliedPairs.forEach((pair) => {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      it(`Android: validate ${tokenRef} @ ${variantName} [${propertyPath.join('.')}]`, async function () {
        ...
        await verifier.verifyTokenApplication(
          ...
          'button-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
    ...
    after(async function () {
      ...
      console.log('\n' + '='.repeat(80));
      console.log('📱 Mobile Button Token Validation Tests Complete');
