...
const widgetKey: Widget = 'popover';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Popover Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-popover.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/popover-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `popover-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.popover?.mapping) {
    console.log(`📂 [popover] Extracting tokens from mapping structure`);

    // For popover, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const popoverMapping = appliedPayload.popover.mapping;

    // Use the first appearance and variant as defaults for popover structure
    ...
    extractTokens(popoverMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in popover payload`);
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Popover page)', async function () {
    
    

    const screenshotName = 'popover-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (popover page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'popover-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Popover Token Validation Tests Complete');
