...
const widgetKey: Widget = 'picture';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Picture Widget', function () {
    ...
    function extractTokens(
        ...
        if ('value' in obj && typeof obj.value === 'string') {
            ...
            // Construct variant name based on appearances/variants structure
            // NOTE: Picture variants in doc are grouped under 'variantGroups.shape' or just 'mapping'
            // We need to match the structure in payload to the variant name key
        ...
        for (const [key, value] of Object.entries(obj)) {
            if (typeof value === 'object' && value !== null) {
                // Handle variantGroups specially if needed, but standard recursion should work
                // if we navigate down the tree correctly.
    ...
    // Payload structure: { picture: { appearances: { default: { variantGroups: { shape: { circle: ... } } }, thumbnail: { mapping: ... } } } }
    if (appliedPayload[widgetKey]?.appearances) {
        console.log(`📂 [${widgetKey}] Extracting tokens from payload structure`);
        const apps = appliedPayload[widgetKey].appearances;

        for (const [appName, appData] of Object.entries(apps)) {
            // 1. Check for variantGroups (e.g. default -> shape -> circle)
            if ((appData as any).variantGroups) {
                for (const [groupName, groupVariants] of Object.entries((appData as any).variantGroups)) {
                    for (const [variantName, variantData] of Object.entries(groupVariants as any)) {
                        extractTokens(variantData, appName, variantName, []);
                    }
                }
            }
            // 2. Check for direct mapping (e.g. thumbnail -> standard/default)
            // If no variantGroups, usually it implies a default/standard variant or direct mapping??
            // Docs say: Thumbnail: standard.
            // Payload example shows: "thumbnail": { "mapping": { ... } }
            // If there's 'mapping', we treat it as variant='standard' (or 'default' depending on config)
            if ((appData as any).mapping) {
                // Check if 'standard' is a valid variant for this appearance
        ...
        console.log(`⚠️  No appearances found in ${widgetKey} payload`);
    }
    ...
    // Per-token validation on ACTUAL apps
