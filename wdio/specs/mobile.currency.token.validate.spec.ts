...
const widgetKey: Widget = 'currency';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Currency Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-currency.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/currency-widget-variants.csv');
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (currency page)', async function () {
    
    

    const screenshotName = 'currency-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (currency page)', async function () {
