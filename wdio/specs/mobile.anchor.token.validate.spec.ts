...
const widgetKey: Widget = 'anchor';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Anchor Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-anchor.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/anchor-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `anchor-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.anchor?.mapping) {
    console.log(`📂 [anchor] Extracting tokens from mapping structure`);

    // For anchor, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const anchorMapping = appliedPayload.anchor.mapping;

    // Use the first appearance and variant as defaults for anchor structure
    ...
    extractTokens(anchorMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in anchor payload`);
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Anchor page)', async function () {
    
    

    const screenshotName = 'anchor-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (anchor page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'anchor-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
      ...
      afterEach(function () {
        try {
          // Construct screenshot name from token + variant to match helper output
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Anchor Token Validation Tests Complete');
