...
const widgetKey: Widget = 'wizard';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Wizard Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-wizard.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/wizard-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `wizard-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.wizard?.mapping) {
    console.log(`📂 [wizard] Extracting tokens from mapping structure`);

    // For wizard, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const wizardMapping = appliedPayload.wizard.mapping;

    // Use the first appearance and variant as defaults for wizard structure
    ...
    extractTokens(wizardMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in wizard payload`);
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Wizard page)', async function () {
    
    

    const screenshotName = 'wizard-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (wizard page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'wizard-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Wizard Token Validation Tests Complete');
