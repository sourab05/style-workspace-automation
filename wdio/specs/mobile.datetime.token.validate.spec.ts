...
const widgetKey: Widget = 'datetime';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Datetime Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-datetime.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/datetime-widget-variants.csv');
  ...
  async function createAndroid(sessionName: string, type: 'baseline' | 'actual'): Promise<Browser> {
    return createAndroidSession({ sessionName, type, runLocal, widgetKey, baselineApps, actualApps });
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (datetime page)', async function () {
    
    

    const screenshotName = 'datetime-page';
    ...
    } catch (err: any) { console.error(`❌ Android baseline capture failed: ${err?.message || err}`); throw err;
    } finally { if (baselineBrowser) { await baselineBrowser.deleteSession(); console.log('🧹 Closed Android baseline session'); } }
    ...
    try {
      ...
      console.log(`   📊 Android screenshot match=${result.match}, diffPixels=${result.diffPixels}, diff%=${result.diffPercentage.toFixed(2)}%`);
    } catch (err: any) { console.error(`Android actual screenshot failed: ${err?.message || err}`); throw err; }
  });

  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (datetime page)', async function () {
    ...
    } catch (err: any) { console.error(`❌ iOS baseline capture failed: ${err?.message || err}`); throw err;
    } finally { if (baselineBrowser) { await baselineBrowser.deleteSession(); console.log('🧹 Closed iOS baseline session'); } }
    ...
    try {
      ...
      console.log(`   📊 iOS screenshot match=${result.match}, diffPixels=${result.diffPixels}, diff%=${result.diffPercentage.toFixed(2)}%`);
    } catch (err: any) { console.error(`iOS actual screenshot failed: ${err?.message || err}`); throw err; }
  });
  ...
  appliedPairs.forEach((pair) => {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(browser, 'android', widgetKey, variantName, tokenRef, {}, 'datetime-page', propertyPath);
      });
      }
      afterEach(function () {
        try {
  ...
  after(async function () {
    if (androidBrowser) { await androidBrowser.deleteSession(); console.log('🧹 Closed Android token validation session'); }
    if (iosBrowser) { await iosBrowser.deleteSession(); console.log('🧹 Closed iOS token validation session'); }
    ...
    if (passedTests + failedTests !== expectedTokenTests) {
      console.warn(`⚠️  Counter mismatch: passed+failed=${passedTests + failedTests}, expected=${expectedTokenTests}`);
    }
    console.log('='.repeat(80) + '\n');
  });
});
