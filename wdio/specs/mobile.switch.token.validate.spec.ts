...
const widgetKey: Widget = 'switch';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Switch Widget', function () {
  ...
  if (widgetPayload?.mapping) {
    console.log(`📂 [${payloadKey}] Extracting tokens from direct mapping`);
    extractTokens(widgetPayload.mapping, 'standard', 'standard', []);
  } else if (widgetPayload?.appearances) {
    const apps = widgetPayload.appearances;
