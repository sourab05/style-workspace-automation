...
const widgetKey: Widget = 'accordion';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Accordion Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-accordion.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/accordion-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `accordion-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.accordion?.mapping) {
    console.log(`📂 [accordion] Extracting tokens from mapping structure`);

    // For accordion, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const accordionMapping = appliedPayload.accordion.mapping;

    // Use the first appearance and variant as defaults for accordion structure
    ...
    extractTokens(accordionMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in accordion payload`);
  }
  ...
  // ---------------------------------------------------------------------------
  // Helper functions to create sessions (using centralized sessionFactory)
  // ---------------------------------------------------------------------------
      ...
      : {
        ...
        capabilities: {
          ...
          'bstack:options': {
            ...
            buildName: `Mobile Token ${type === 'baseline' ? 'Baseline' : 'Validation'} - Android (Accordion)`,
            sessionName,
            debug: true,
            networkLogs: true,
            video: true,
  ...
  appliedPairs.forEach((pair) => {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        // RN inspector command can leave the page in a bad state; only wait for widget before first styles fetch
        ...
        await verifier.verifyTokenApplication(
          ...
          'accordion-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
  // Cleanup & summary (once per suite, not per token)
  // ---------------------------------------------------------------------------
  ...
  after(async function () {
    ...
    const platformDir = path.join(outputDir, 'android');
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Accordion Token Validation Tests Complete');
