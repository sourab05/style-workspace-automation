...
const widgetKey: Widget = 'message';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Message Widget', function () {
  this.timeout(300000);
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-message.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/message-widget-variants.csv');
  ...
  if (appliedPayload.message?.appearances) {
    // Iterate through configured appearances
    for (const appearance of widgetConfig.appearances) {
      const appearanceData = appliedPayload.message.appearances[appearance];
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (message page)', async function () {
    
    

    const screenshotName = 'message-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (message page)', async function () {
