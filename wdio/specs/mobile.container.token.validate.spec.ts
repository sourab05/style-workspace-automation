...
const widgetKey: Widget = 'container';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Container Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-container.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/container-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `container-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (widgetPayload?.appearances) {
    ...
    for (const appearance of widgetConfig.appearances) {
      ...
      // For container, extract tokens from the mapping object within each appearance
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Container page)', async function () {
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (container page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'container-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
