...
const widgetKey: Widget = 'chips';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Chips Widget', function () {
  ...
  // Payload: { chips: { appearances: { filled: { variantGroups: { status: { primary: ... } } } } } } }
  if (appliedPayload[widgetKey]?.appearances) {
    console.log(`📂 [${widgetKey}] Extracting tokens from appearances structure`);
    const apps = appliedPayload[widgetKey].appearances;

    for (const [appName, appData] of Object.entries(apps)) {
      if (!widgetConfig.appearances.includes(appName as Appearance)) {
        continue;
      }

      if ((appData as any).variantGroups) {
        const allowedVariants = widgetConfig.variants[appName as Appearance] || [];
        for (const [, groupVariants] of Object.entries((appData as any).variantGroups)) {
          for (const [variantName, variantData] of Object.entries(groupVariants as any)) {
            if (!allowedVariants.includes(variantName as any)) {
              continue;
            }
  ...
  // Helper function to create Android session
