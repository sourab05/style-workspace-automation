...
describe('Mobile Token Validation - Chips Widget', function () {
  ...
  // Payload structure: { chips: { appearances: { filled: { variantGroups: { status: { primary: ... } } }, elevated: { mapping: ... } } } }
  if (appliedPayload[widgetKey]?.appearances) {
    ...
    for (const [appName, appData] of Object.entries(apps)) {
      // 1. Check for variantGroups (e.g. filled -> status -> primary)
      ...
      // 2. Check for direct mapping (e.g. elevated -> mapping)
