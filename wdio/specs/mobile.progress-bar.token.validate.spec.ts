...
const widgetKey: Widget = 'progress-bar';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Progress Bar Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-progress-bar.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/progress-bar-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `progress-bar-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Progress Bar page)', async function () {
    
    

    const screenshotName = 'progress-bar-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (progress-bar page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'progress-bar-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Progress Bar Token Validation Tests Complete');
