...
const widgetKey: Widget = 'search';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Search Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-search.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/search-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `search-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload.search?.mapping) {
    console.log(`📂 [search] Extracting tokens from mapping structure`);

    // For search, we extract tokens from the mapping object directly
    // since it contains all properties regardless of appearance/variant
    const searchMapping = appliedPayload.search.mapping;

    // Use the first appearance and variant as defaults for search structure
    ...
    extractTokens(searchMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in search payload`);
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Search page)', async function () {
    
    

    const screenshotName = 'search-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (search page)', async function () {
    ...
    describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
      if (shouldRunAndroid) {
        ...
        await verifier.verifyTokenApplication(
          ...
          'search-page', // Use the full-page screenshot baseline for all token validation tests
          propertyPath
        );
      });
      }
  ...
  // ---------------------------------------------------------------------------
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Search Token Validation Tests Complete');
