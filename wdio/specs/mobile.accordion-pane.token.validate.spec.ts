...
const widgetKey: Widget = 'accordion-pane';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Accordion Pane Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-accordion-pane.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/accordion-pane-widget-variants.csv');
  ...
  function extractTokens(
    ...
    if ('value' in obj && typeof obj.value === 'string') {
      ...
      const variantName = `accordion-pane-${appearance}-${variant}-${stateSuffix}`;
      appliedPairs.push({
        tokenRef: obj.value,
        variantName,
  ...
  if (appliedPayload['accordion-pane']?.mapping) {
    console.log(`📂 [accordion-pane] Extracting tokens from mapping structure`);

    const accordionPaneMapping = appliedPayload['accordion-pane'].mapping;
    ...
    extractTokens(accordionPaneMapping, defaultAppearance, defaultVariant, []);
  } else {
    console.log(`⚠️  No mapping found in accordion-pane payload`);
  }
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (Accordion page)', async function () {
    ...
    const screenshotName = 'accordion-pane-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (Accordion Pane page)', async function () {
      ...
      afterEach(function () {
        try {
          ...
          if (fs.existsSync(baseFile)) {
            try { allure.addAttachment('Base Screenshot', fs.readFileSync(baseFile), 'image/png'); } catch (e) {}
          }
          if (fs.existsSync(actualFile)) {
            try { allure.addAttachment('Actual Screenshot', fs.readFileSync(actualFile), 'image/png'); } catch (e) {}
          }
          if (fs.existsSync(diffFile)) {
            try { allure.addAttachment('Diff Screenshot', fs.readFileSync(diffFile), 'image/png'); } catch (e) {}
          }

          const verdictPath = path.join(
            process.cwd(),
            'artifacts', 'mobile-styles', widgetKey,
  ...
  after(async function () {
    ...
    console.log('\n' + '='.repeat(80));
    console.log('📱 Mobile Accordion Pane Token Validation Tests Complete');
