...
const widgetKey: Widget = 'select';
type Browser = WebdriverIO.Browser;

describe('Mobile Token Validation - Select Widget', function () {
  ...
  const batchPayloadPath = path.join(process.cwd(), '.test-cache/batch-payload-select.json');
  ...
  const csvPath = path.join(process.cwd(), 'tests/testdata/mobile/select-widget-variants.csv');
  ...
  baselineScreenshotIt('android')('Android baseline vs actual screenshot (select page)', async function () {
    
    

    const screenshotName = 'select-page';
  ...
  baselineScreenshotIt('ios')('iOS baseline vs actual screenshot (select page)', async function () {
