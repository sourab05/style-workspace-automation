...
const widgetKey: Widget = 'bottomsheet';
type Browser = WebdriverIO.Browser;

const BOTTOMSHEET_TRIGGER_ID = 'button1_a';

async function ensureBottomsheetVisible(browser: Browser, widgetPage: MobileWidgetPage): Promise<void> {
    const isVisible = await widgetPage.isWidgetVisible(browser, widgetKey);
    if (!isVisible) {
        console.log(`   📱 Bottomsheet not visible, tapping trigger button (${BOTTOMSHEET_TRIGGER_ID})...`);
        const trigger = await browser.$(`~${BOTTOMSHEET_TRIGGER_ID}`);
...
describe('Mobile Token Validation - Bottomsheet Widget', function () {
    ...
    // Extract tokens based on direct mapping
    if (appliedPayload[widgetKey]?.mapping) {
        console.log(`📂 [${widgetKey}] Extracting tokens from mapping structure`);

        const mapping = appliedPayload[widgetKey].mapping;
        ...
        extractTokens(mapping, defaultAppearance, defaultVariant, []);
    } else {
        console.log(`⚠️  No mapping found in ${widgetKey} payload`);
    }
        ...
        describe(`Token Validate: ${tokenRef} Property: ${propertyPath.join('.')}`, function () {
            it(`Android: validate ${tokenRef} @ ${variantName} [${propertyPath.join('.')}]`, async function () {
                ...
                if (!androidBrowser) {
                    ...
                    await ensureBottomsheetVisible(androidBrowser, widgetPageWarmup);
                }
                ...
                await ensureBottomsheetVisible(browser, widgetPage);
            ...
            it(`iOS: validate ${tokenRef} @ ${variantName} [${propertyPath.join('.')}]`, async function () {
                ...
                if (!iosBrowser) {
                    ...
                    await ensureBottomsheetVisible(iosBrowser, widgetPageWarmup);
                }
